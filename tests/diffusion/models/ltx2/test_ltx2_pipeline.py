...
def test_ltx_versions_share_runtime_without_cross_version_inheritance():
    ...
    assert issubclass(LTX2Pipeline, LTX2LoraLoaderMixin)
    assert issubclass(LTX23Pipeline, LTXPipelineRuntime)
    assert issubclass(LTX2TwoStagesPipeline, LTXPipelineRuntime)
    assert issubclass(LTX2TwoStagesPipeline, LTX2LoraLoaderMixin)
...
def test_ltx2_two_stage_restores_lora_and_scheduler_on_stage2_failure(monkeypatch):
    request_inputs = LTXRequestInputs(
        prompt="prompt",
        negative_prompt="negative",
        height=32,
    ...
    original_scheduler = SimpleNamespace(config={"use_dynamic_shifting": True})
    stage2_scheduler = object()
    pipeline = object.__new__(LTX2TwoStagesPipeline)
    torch.nn.Module.__init__(pipeline)
    pipeline.distilled = False
    pipeline.model_path = "/models/LTX-2"
    ...
    def run_phase(req, inputs, *, prompt_context=None, **kwargs):
        if inputs is request_inputs:
            return LTXPhaseResult(
                forward_context=SimpleNamespace(prompt_context=object()),
                video=torch.tensor([1.0]),
                audio=torch.tensor([2.0]),
            )
        assert prompt_context is None
        assert pipeline.scheduler is stage2_scheduler
        raise RuntimeError("stage 2 failed")

    def load_lora_weights(path, adapter_name):
        calls.append(("load", path, adapter_name))

    def unload_lora_weights(adapter_name):
        calls.append(("unload", adapter_name))

    pipeline.upsample_pipe = FakeUpsampler()
    object.__setattr__(pipeline, "run_phase", run_phase)
    object.__setattr__(pipeline, "load_lora_weights", load_lora_weights)
    object.__setattr__(pipeline, "unload_lora_weights", unload_lora_weights)
    monkeypatch.setattr(
        "vllm_omni.diffusion.models.ltx2.pipeline_ltx2_two_stage.FlowMatchEulerDiscreteScheduler.from_config",
        lambda config, **kwargs: stage2_scheduler,
    )

    with pytest.raises(RuntimeError, match="stage 2 failed"):
        pipeline._run_two_stage(
            SimpleNamespace(),
            request_inputs,
            noise_scale=0.0,
            timesteps=None,
    ...
    assert calls == [
        (
            "load",
            "/models/LTX-2/ltx-2-19b-distilled-lora-384.safetensors",
            "stage_2_distilled",
        ),
        ("unload", "stage_2_distilled"),
    ]
